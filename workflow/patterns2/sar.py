...
fig,ax=plt.subplots(nrows=len(Pheno['phenotype']),ncols=len(cuts),figsize=(40,15))
fig.patch.set_facecolor('#FFFFFF')

R={}

title=['Low thr.$\\approx 10^{-6}$','Mid thr.$\\approx 10^{-5}$','jgjjg','High thr.$\\approx 10^{-4}$']
...
for p,i in zip(['H','U'],range(len(Pheno['phenotype']))):
    ...
    for c,j in zip(cuts,range(len(cuts))):
        ...
        md=Pheno['T'][p][c].form['original'].sum(axis=0)
        od=Pheno['T'][p][c].form['binary'].sum(axis=0)
        ...
        for r in range(replicas):
            ...
            ax[i][j].scatter(AO_model['x'],AO_model['y'],s=80,color='grey',alpha=0.1,zorder=1,edgecolor='none')

            R2.loc[c,r]=an.compare_neutral_curves(AO_data,AO_model,delta=0.2,n_bins=15)
...
fig.savefig(os.path.join('../..',PLOT_DIR,'sar'), transparent=False, dpi=150,bbox_inches='tight')
...
Rtot.to_csv(os.path.join('../..',TEST_DIR,'R2_sar.csv'),index=True)
